...
- **`addon/TalentAdvisor`**: says where the next talent point goes and which
  ...
  - **Two test runs, neither of them needing a game.**
    ...
    itself inside [`tools/wowsim`](tools/wowsim/README.md), a stand-in 3.3.5
    client: log in, answer the picker, level up, click Learn, drop things in
    the bags, click a row. 35 scenarios, seven of them about layout.
  - **The stand-in client is not addon-specific** - it loads any addon from its
    `.toc`, Lua and XML alike, lays the frames out against the client's own
    fonts, and draws the result with the client's own art. It is documented on
    its own page; what matters here is that four of TalentAdvisor's bugs were
    found by it and could not have been found any other way: a heading indented
    differently from its neighbours, a panel with under a pixel of clearance
    above its footer, a title that wrapped onto the icon below it, and three
    descriptions that ran out of their rows once the font was the real one.
